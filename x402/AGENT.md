...
- `execute-endpoint` in the default sponsored mode refuses to sign (`Insufficient sBTC|STX for a sponsored x402 payment`) when the wallet cannot cover the price in the chosen asset; no STX is needed for gas. If the Stacks API cannot be read the check is skipped and the payment proceeds, so an unfunded payment can still fail at settlement
...
- Repeating an identical paid request is refused with the earlier `txid` only while that earlier payment is unresolved (its paid request failed, timed out, or is still in flight), for up to `X402_DEDUP_TTL_SECONDS` (default 15 min). A request whose earlier call got a 2xx is paid normally. Never work around the guard by varying parameters or editing the state file; verify the txid instead
- Do not run a direct x402 payment in parallel with another write from the same wallet: direct mode does not use the shared nonce tracker, so the two can collide on a nonce
...
| "an identical request was already paid Ns ago (txid ...)" | Duplicate guard: an identical earlier payment is unresolved (failed/timed-out/in-flight paid request) | Verify that txid; if it is absent from mempool and chain, wait out the TTL — do not vary parameters or edit the state file to get around it |
| "the paid request never reached the server ... Nothing was sent or paid" | DNS, connection-refused or TLS failure before the request was sent | Safe to retry once the endpoint is reachable; the guard records were released |
