...
- `execute-endpoint`: read `response` for the API response data; payment metadata may also be attached internally as canonical status, decision, `paymentId`, and `checkUrl`. `x402-api` remains an immediate pay-per-call exception and does not establish a generic local payment-status route contract.
